...
// Initial configurations for key task types
const initialConfigurations: Partial<Record<DocumentTaskType, TaskConfiguration>> = {
  [DocumentTaskType.CREATE_DOCUMENT]: {
...
// Complete Task Configurations - fill in remaining types with defaults
export const TaskConfigurations: Record<DocumentTaskType, TaskConfiguration> = {
  ...initialConfigurations,
  ...Object.fromEntries(
    Object.values(DocumentTaskType)
      .filter(type => !initialConfigurations[type as DocumentTaskType])
      .map(type => [
        type,
        {
...
} as Record<DocumentTaskType, TaskConfiguration>;

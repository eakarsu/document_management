...
# Richmond Document Management System - One Command Startup
# Simply run: ./start.sh
# This script does EVERYTHING to start the complete system
...
main() {
    print_header "Richmond Document Management System - Complete Setup"
    echo "=================================================="
    ...
    print_info "Checking database data..."
    
    # Better way to check if database has users
    user_exists=$(cd backend && npx prisma client generate >/dev/null 2>&1 && node -e "
    ...
    if [ "$user_exists" = "0" ] || [ "$user_exists" = "" ]; then
        ...
        if (cd database && npm run seed >/dev/null 2>&1); then
            ...
            user_exists="3"  # We know we created 3 demo users
        else
...
# This file is automatically generated by start.sh
...
EOF
    ...
    # Show database info
    doc_count=$(cd backend && node -e "
        const { PrismaClient } = require('@prisma/client');
        const prisma = new PrismaClient();
        prisma.document.count().then(count => {
            console.log(count);
            process.exit(0);
        }).catch(() => {
            console.log('?');
    ...
    " 2>/dev/null || echo "?")
    echo "   ✅ Database: $user_exists users, $doc_count documents"
    echo ""
...
case "${1:-}" in
    --help|-h)
        echo "Richmond Document Management System - One Command Setup"
        echo "======================================================"
        echo ""
        echo "Usage: $0 [OPTION]"
        echo ""
        echo "Options:"
        echo "  (no option)    Start the complete DMS system"
